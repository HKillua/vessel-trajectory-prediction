...
class Trajectron(torch.nn.Module):
    def __init__(self, cfg):
        ...
        with open(
            "src/data/TP/Trajectron-plus-plus/experiments/pedestrians/models/eth_vel/config.json",
            "r",
            encoding="utf-8",
        ) as conf_json:
        ...
        for node_type in train_env.NodeType:
            ...
            self.optimizer[node_type] = optim.Adam(
                [
                    ...
                    {
                        "params": self.model_registrar.get_name_match(
                            "map_encoder"
                        ).parameters(),
                        "lr": 0.0008,
                    },
    ...
    def predict(self, data_dict, return_prob=False):
        ...
        model = self.node_models_dict[node_type]
        y_dist, predictions = model.predict(
            inputs=x_t,
            inputs_st=x_st_t,
            first_history_indices=first_history_index,
            neighbors=restore(neighbors_data_st),
        ...
        if return_prob:
            ...
            y_dist.log_prob(
                torch.rand_like(predictions).tile(1, 10000, 1, 1)
            )  # calculate the probability for fair time comparison
