...
class ARFlow(nn.Module):
    def __init__(self, cfg: CfgNode) -> None:
        ...
        div_term = torch.exp(
            torch.arange(0, self.pe_dim, 2) * (-math.log(10000.0) / self.pe_dim)
        )
    ...
    def predict(self, data_dict: Dict) -> Dict:
        ...
        for k in range(self.pred_len):
            ...
            dec_output = self.transformer.decoder(
                self.decoder_input(torch.cat(current_dec_inputs, dim=0)), enc_out
            )[-1][None]
            sample_num = 10000
...
class Flow(nn.Module):
    ...
    def sample_with_log_prob(self, sample_shape=torch.Size(), cond=None):
        ...
        return (
            sample,
            torch.sum(self.base_dist.log_prob(u), dim=-1) + sum_log_abs_det_jacobians,
        )
