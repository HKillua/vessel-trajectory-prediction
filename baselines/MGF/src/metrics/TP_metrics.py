...
class TP_metrics(object):
    ...
    def compute_mse(self, dict_list: List) -> Dict:
        ade, fde = [], []
        for data_dict in dict_list:
            ade.append(
                displacement_error(
        ...
        ade = evaluate_helper_mean(ade)
        fde = evaluate_helper_mean(fde)
    ...
    def output_to_trajectories(self, data_dict: Dict) -> Dict:
        ...
        if "position" in self.state.keys():
            ...
            data_dict["gt"] = (
                torch.cumsum(data_dict["gt"], dim=1) * data_dict["dt"][:, None, None]
                + data_dict["obs"][:, -1:, 0:2]
            )
            for k in data_dict.keys():
                if k[0] == "prob" and type(data_dict[k]) == torch.Tensor:
...
def evaluate_helper_mean(error):
    error = torch.stack(error, dim=0)
    min_error_sum = torch.min(error, dim=0)[0].mean(dim=0)
    # mean_error_sum = error.mean(dim=0).mean(dim=0)
    return min_error_sum
