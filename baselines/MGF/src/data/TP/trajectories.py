...
class TrajectoryDataset(Dataset):
    ...
    def __init__(
        ...
        self.mean = torch.from_numpy(np.mean(seq_list, axis=(0, 2))).to(
            dtype=torch.float32
        )
        self.std = torch.from_numpy(np.std(seq_list, axis=(0, 2))).to(
            dtype=torch.float32
        )
...
class SimulatedForkTrajectory(Dataset):
    ...
    def __init__(
        ...
        self.traj = []
        for sgn in [1, -1]:
            traj_flat_x = torch.arange(-self.obs_len - 1.0, 0.0, 1.0, dtype=torch.float)
            traj_flat_y = torch.zeros_like(traj_flat_x, dtype=torch.float)
            traj_flat = torch.cat([traj_flat_x[None, :], traj_flat_y[None, :]], dim=0)

            dist = torch.arange(1.0, self.pred_len + 1.0, 1.0, dtype=torch.float)
    ...
    def __getitem__(self, index):
        ...
        traj += torch.normal(
            mean=torch.zeros_like(traj), std=torch.ones_like(traj) * self.sigma
        )
        ...
        out = [
            index,
            traj[:, : self.obs_len][None, :, :],
            traj[:, self.obs_len :][None, :, :],
            traj_rel[:, : self.obs_len][None, :, :],
            traj_rel[:, self.obs_len :][None, :, :],
            non_linear,
            loss_mask,
            self.max,
...
class SimulatedCrossTrajectory(Dataset):
    ...
    def __init__(
        ...
        for sgn in [1, -1]:
            ...
            dist = torch.cat(
                [
                    torch.arange(1.0, 3.0, 1.0, dtype=torch.float),
                    torch.arange(2.0, -self.pred_len + 4.0, -1.0, dtype=torch.float),
                ]
            )
            traj_ramp_y = dist * np.sin(sgn * self.theta, dtype=np.float32)
            traj_ramp = torch.cat([traj_ramp_x[None, :], traj_ramp_y[None, :]], dim=0)
