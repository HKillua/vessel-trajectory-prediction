...
class FlowMatcher(nn.Module):
    ...
    def sample(self, x_data, num_trajs, return_all_states=False):
        ...
        batch_size = int(x_data['batch_size'])
    ...
    def p_losses(self, x_data, log_dict=None):
        ...
        # Ship: target-ship-only loss
        if 'target_ship_idx' in x_data:
            target_idx = x_data['target_ship_idx']  # [B]
            ti = target_idx.view(B, 1, 1, 1).expand(B, K, 1, T)
            denoising_error_per_agent = denoising_error_per_agent.gather(2, ti)  # [B, K, 1, T]
            # Also extract target ship cls logits
            ti_cls = target_idx.view(B, 1, 1).expand(B, K, 1)
            denoiser_cls = denoiser_cls.gather(2, ti_cls)  # [B, K, 1]
            A = 1
