...
class MTRDecoder(nn.Module):
    ...
    def forward(self, query_token, time_emb=None, interaction_bias=None, padding_mask=None):
        """
        @param query_token:       [B, K, A, D]
        @param time_emb:          [B, D]
        @param interaction_bias:  [B*H, A, A] or None — additive attention bias for A-to-A
        @param padding_mask:      [B, A] bool (True=real) or None
        """
        ...
        if padding_mask is not None:
            pad_mask_a = (~padding_mask).unsqueeze(1).expand(B, K, A).reshape(B * K, A)

        inter_bias_a = None
        if interaction_bias is not None:
            H = interaction_bias.shape[0] // B
            inter_bias_a = interaction_bias.reshape(B, H, A, A)
            inter_bias_a = inter_bias_a.unsqueeze(1).expand(B, K, H, A, A)
            inter_bias_a = inter_bias_a.reshape(B * K * H, A, A)
        ...
        for i in range(self.num_blocks):
            ...
            if inter_bias_a is not None:
                cur_query = _forward_layer_no_fastpath(
                    self.self_attn_A[i], cur_query,
                    src_mask=inter_bias_a, src_key_padding_mask=pad_mask_a,
                )
            else:
                cur_query = self.self_attn_A[i](
                    cur_query, src_key_padding_mask=pad_mask_a,
                )
