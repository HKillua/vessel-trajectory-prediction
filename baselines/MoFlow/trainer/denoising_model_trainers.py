...
def build_scheduler(optimizer, opt_cfg, total_iters_each_epoch):
    ...
    if opt_cfg.get('SCHEDULER', None) == 'cosineAnnealingLRwithWarmup':
        ...
        scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=opt_cfg.DECAY_STEP * total_iters_each_epoch, gamma=opt_cfg.DECAY_GAMMA)
    elif opt_cfg.get('SCHEDULER', None) == 'cosineAnnealingLR':
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=total_epochs * total_iters_each_epoch, eta_min=opt_cfg.LR_CLIP)
    else:
        scheduler = None
    return scheduler
...
class Trainer(object):
        ...
        with tqdm(initial = self.step, total = self.train_num_steps, disable = not accelerator.is_main_process) as pbar:
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    data = {k: v.to(self.device) if isinstance(v, torch.Tensor) else v for k, v in next(self.dl).items()}
    ...
    def sample_from_denoising_model(self, data):
        ...
        elif self.cfg.get('data_norm', None) in ('original', 'pre_normalized'):
            pass
        else:
    ...
    def save_latent_states(self, t_seq_ls, y_t_seq_ls, y_pred_data_ls, x_data_ls, pred_score_ls, file_name):
        ...
        elif self.cfg.dataset == 'ship':
            keys_to_save = ['past_traj', 'fut_traj', 'past_traj_original_scale', 'fut_traj_original_scale']
        else:
    ...
    def eval_dataloader(self, testing_mode=False, training_err_check=False):
        ...
        ### record running time
        use_cuda_timing = torch.cuda.is_available() and str(self.device) != 'cpu'
        if use_cuda_timing:
            start = torch.cuda.Event(enable_timing=True)
            end = torch.cuda.Event(enable_timing=True)
            start.record()

        ship_mean_xy, ship_std_xy = None, None
        ...
        for i_batch, data in enumerate(dl): 
            ...
            if self.cfg.dataset == 'ship':
                # Ship: extract target ship only
                target_idx = data['target_ship_idx']  # [B]
                B_cur = int(data['batch_size'])
                A_cur = data['past_traj'].shape[1]
                # pred_traj is [B*A, K, F, 2], reshape to [B, A, K, F, 2]
                pred_traj_ba = pred_traj.reshape(B_cur, A_cur, -1, self.cfg.future_frames, 2)
                ti = target_idx.view(B_cur, 1, 1, 1, 1).expand(-1, 1, pred_traj_ba.shape[2], self.cfg.future_frames, 2)
                pred_traj = pred_traj_ba.gather(1, ti).squeeze(1)  # [B, K, F, 2]

                pred_traj_t_ba = pred_traj_t.reshape(B_cur, A_cur, pred_traj_t.shape[1], -1, self.cfg.future_frames, 2)
                ti_t = target_idx.view(B_cur, 1, 1, 1, 1, 1).expand(-1, 1, pred_traj_t_ba.shape[2], pred_traj_t_ba.shape[3], self.cfg.future_frames, 2)
                pred_traj_t = pred_traj_t_ba.gather(1, ti_t).squeeze(1)  # [B, S, K, F, 2]

                # fut_traj_original_scale: [B, A, F, 2] → extract target
                fut_all = data['fut_traj_original_scale']  # [B, A, F, 2]
                ti_fut = target_idx.view(B_cur, 1, 1, 1).expand(-1, 1, self.cfg.future_frames, 2)
                fut_traj = fut_all.gather(1, ti_fut).squeeze(1)  # [B, F, 2]

                if ship_mean_xy is not None:
                    pred_traj = pred_traj * ship_std_xy + ship_mean_xy
                    pred_traj_t = pred_traj_t * ship_std_xy + ship_mean_xy
                    fut_traj = fut_traj * ship_std_xy + ship_mean_xy
                ...
                distances_t = (pred_traj_t - fut_traj_gt.unsqueeze(1)).norm(p=2, dim=-1)
            else:
            ...
            if self.cfg.dataset == 'nba':
                ...
                F = self.cfg.future_frames
                ship_eval_frames = [max(1, F // 4), max(1, F // 2), max(1, 3 * F // 4), F]
                freq = None
                factor_time = 1
            
            for time in range(1, 5):
                end_frame = ship_eval_frames[time - 1] if freq is None else int(time * freq)
                ade, fde, ade_avg, fde_avg = self.compute_ADE_FDE(distances, end_frame)
                jade, jfde, jade_avg, jfde_avg = self.compute_JADE_JFDE(distances, end_frame) 
                a_var, f_var = self.compute_avar_fvar(pred_traj, end_frame)
                masd = self.compute_MASD(pred_traj, end_frame)
            ...
            if self.save_samples:
                ...
                if self.cfg.dataset == 'ship':
                    pred_traj = pred_traj.unsqueeze(2)  # [B, K, F, 2] -> [B, K, 1, F, 2]
                else:
                    pred_traj = rearrange(pred_traj, '(b a) k f d -> b k a f d', b=bs)  # [B*A, K, F, 2] -> [B, K, A, F, 2]
        ...
        if use_cuda_timing:
            ...
            self.logger.info(f'Total runtime: {elapsed_ms:5f} ms')
            self.logger.info(f'Runtime per scene: {elapsed_ms/len(dl.dataset):5f} ms')
        ...
        if self.cfg.dataset == 'ship':
            eval_labels = [f'f{ship_eval_frames[i]}' for i in range(4)]
        else:
            eval_labels = [f'{(i+1)*factor_time:.1f}s' for i in range(4)]

        if not testing_mode: 
            self.logger.info(f'{self.step}/{self.train_num_steps}, running inference on {num_trajs} agents (trajectories)')
            for time in range(4):
                lbl = eval_labels[time]
                if self.tb_log:
                    self.tb_log.add_scalar(f'eval_{status}/ADE_min_{lbl}', performance['ADE_min'][time]/num_trajs, cur_epoch)
                    self.tb_log.add_scalar(f'eval_{status}/FDE_min_{lbl}', performance['FDE_min'][time]/num_trajs, cur_epoch)
                    self.tb_log.add_scalar(f'eval_{status}/ADE_avg_{lbl}', performance['ADE_avg'][time]/num_trajs, cur_epoch)
                    self.tb_log.add_scalar(f'eval_{status}/FDE_avg_{lbl}', performance['FDE_avg'][time]/num_trajs, cur_epoch)
                    self.tb_log.add_scalar(f'eval_{status}/JADE_min_{lbl}', performance_joint['JADE_min'][time]/num_trajs, cur_epoch)
                    self.tb_log.add_scalar(f'eval_{status}/JFDE_min_{lbl}', performance_joint['JFDE_min'][time]/num_trajs, cur_epoch)
                    self.tb_log.add_scalar(f'eval_{status}/JADE_avg_{lbl}', performance_joint['JADE_avg'][time]/num_trajs, cur_epoch)
                    self.tb_log.add_scalar(f'eval_{status}/JFDE_avg_{lbl}', performance_joint['JFDE_avg'][time]/num_trajs, cur_epoch)

        # print out the performance
        for time in range(4):
            lbl = eval_labels[time]
            self.logger.info('--ADE_min({}): {:.7f}\t--FDE_min({}): {:.7f}'.format(
                lbl, performance['ADE_min'][time]/num_trajs, lbl, performance['FDE_min'][time]/num_trajs))

      
        for time in range(4):
            lbl = eval_labels[time]
            self.logger.info('--ADE_avg({}): {:.7f}\t--FDE_avg({}): {:.7f}'.format(
                lbl, performance['ADE_avg'][time]/num_trajs, lbl, performance['FDE_avg'][time]/num_trajs))

        for time in range(4):
            lbl = eval_labels[time]
            self.logger.info('--AVar({}): {:.7f}\t--FVar({}): {:.7f}'.format(
                lbl, performance['A_var'][time]/num_trajs, lbl, performance['F_var'][time]/num_trajs))
        
        for time in range(4):
            lbl = eval_labels[time]
            self.logger.info('--MASD({}): {:.7f}'.format(
                lbl, performance['MASD'][time]/num_trajs))
            
        # print out the joint performance
        for time in range(4):
            lbl = eval_labels[time]
            self.logger.info('--JADE_min({}): {:.7f}\t--JFDE_min({}): {:.7f}'.format(
                lbl, performance_joint['JADE_min'][time]/num_trajs, lbl, performance_joint['JFDE_min'][time]/num_trajs))
        
        for time in range(4):
            lbl = eval_labels[time]
            self.logger.info('--JADE_avg({}): {:.7f}\t--JFDE_avg({}): {:.7f}'.format(
                lbl, performance_joint['JADE_avg'][time]/num_trajs, lbl, performance_joint['JFDE_avg'][time]/num_trajs))
