...
class GPT4STP(nn.Module):
    ...
    def __init__(self, configs, device):
        ...
        # Path to local GPT-2 model (downloaded from huggingface.co/gpt2)
        gpt2_path = getattr(configs, 'gpt2_model_path', None) or '/home/wangguangjie/djs/baseline/gpt2_model'

        if configs.is_gpt:
            if configs.pretrain:
                try:
                    self.gpt2 = GPT2Model.from_pretrained(gpt2_path, output_attentions=True,
                                                          output_hidden_states=True, attn_implementation="eager")
                    self.tokenizer = GPT2TokenizerFast.from_pretrained(gpt2_path,
                                                                   trust_remote_code=True, local_files_only=True)
                    print(f"Loaded GPT-2 from {gpt2_path}")
                except Exception as e:
                    print(f"Failed to load GPT-2 from {gpt2_path}: {e}")
                    print("Using random initialized GPT-2 instead")
                    self.gpt2 = GPT2Model(GPT2Config())
                    self.tokenizer = GPT2TokenizerFast(vocab_file=gpt2_path + '/vocab.json',
                                                       merges_file=gpt2_path + '/merges.txt')
            else:
                print("------------------no pretrain------------------")
                self.gpt2 = GPT2Model(GPT2Config())
                # Create tokenizer from local vocab files
                import os
                if os.path.exists(gpt2_path + '/vocab.json'):
                    self.tokenizer = GPT2TokenizerFast(vocab_file=gpt2_path + '/vocab.json',
                                                       merges_file=gpt2_path + '/merges.txt')
                else:
        ...
        self.geohash_encoder = nn.Sequential(
            nn.Conv1d(in_channels=configs.seq_len, out_channels=32, kernel_size=3, padding=1),
            nn.ReLU(),
