...
def find_interaction_scenes(df, config):
    ...
    中按实际共存时间筛选。这样避免交集操作把多船场景缩为 2 艘。
    """
...
def extract_all_scenes():
    ...
    if not all_data:
        print("错误：未找到预处理数据，请先运行 04_preprocess_all_regions.py")
        return False
        
    combined = pd.concat(all_data, ignore_index=True)
    print(f"\n总数据量: {len(combined)} 行, {combined['mmsi'].nunique()} 艘船")
    
    # 构建 heading 质量查找表，按船过滤
    min_heading_quality = config.get("preprocessing", {}).get("min_heading_quality", 0.0)
    heading_quality_map = {}
    if "heading_quality" in combined.columns:
        heading_quality_map = combined.groupby("mmsi")["heading_quality"].first().to_dict()
        n_before = combined["mmsi"].nunique()
        if min_heading_quality > 0:
            low_q_ships = {m for m, q in heading_quality_map.items() if q < min_heading_quality}
            combined = combined[~combined["mmsi"].isin(low_q_ships)]
            n_after = combined["mmsi"].nunique()
        ...
        print(f"去重：{n_before_dedup} -> {len(all_scene_data)} 场景 (移除 {n_removed} 个重复场景)")
